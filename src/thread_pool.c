...
{
	CThread_pool_t *pool = NULL;
	do 
	{
		pool = malloc(sizeof(CThread_pool_t));
		if (NULL == pool)
		{
		...
		{
			pthread_create(&(pool->pthreads[i]), NULL, CThread_pool_function, (void *)pool);
		}
...
int CThread_worker_add(CThread_pool_t* pool, void* (*callback_function)(void *arg), void *arg)
{
	...
	CThread_worker_t *pjob =(CThread_worker_t*) malloc(sizeof(CThread_worker_t));
	if (NULL == pjob)
	{
		pthread_mutex_unlock(&(pool->mutex));
		return -1;
